...
# Analyze Papers page
    ...
    def analyze_handwriting(image, question, model_answer):
        ...
        prompt = f"""
        You are an expert exam grader analyzing handwritten student answers.
        
        QUESTION: {question}
        
        MODEL ANSWER: {model_answer}
        
        Analyze the handwritten answer in the image and provide the following:
        1. Transcription of the handwritten text
        2. Evaluation of the answer compared to the model answer (correct/partially correct/incorrect)
        3. Justification for your evaluation
        4. Suggested score as a percentage (0-100%)
        
        Format your response as JSON:
        {{
            "transcription": "The transcribed handwritten text",
    ...
    for i, tab in enumerate(tabs):
        with tab:
            ...
            uploaded_file = st.file_uploader(
                f"Upload handwritten answer for Question {i+1}",
                type=["jpg", "jpeg", "png"],
                key=f"upload_{i}"
            )
            ...
            if uploaded_file is not None:
                ...
                st.image(image, caption=f"Uploaded answer for Question {i+1}", width=400)
                uploaded_images[i] = image
                ...
                st.info(f"Please upload the handwritten answer for Question {i+1}")
    
    # Analyze all button
    if uploaded_images and len(uploaded_images) == len(st.session_state.questions_answers):
        if st.button("Analyze All Remaining Questions"):
            progress_bar = st.progress(0)
            
            for i, qa in enumerate(st.session_state.questions_answers):
                if i not in uploaded_images or i >= len(st.session_state.analyzed_results) or st.session_state.analyzed_results[i] is None:
                    continue
...
# Results & Reports page
    ...
    for i, result in enumerate(st.session_state.analyzed_results):
        ...
        with st.expander(f"Question {i+1} - {result['actual_marks']}/{result['max_marks']} marks"):
            ...
            st.markdown(f"<div class='{evaluation_class}'>", unsafe_allow_html=True)
            st.write(f"**Student's Answer (Transcribed):** {result['transcription']}")
